...
export default {
	...
	mounted() {
		...
		if(this.$route.query.reset_token) {
			this.resetToken.token = this.$route.query.reset_token
			this.$http.post('/api/reset-password/validate-token', this.resetToken)
			.then((response) => {
		...
		resetPassword() {
			...
			if(!this.$v.$invalid) {
				...
				.then((response) => {
					...
					else this.$router.push({path: '/signin?reset_password=done'}) 
				})
				.catch((error) => {
					console.log(error)
					this.disableButton = ''
