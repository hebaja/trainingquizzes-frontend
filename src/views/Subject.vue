			...
			<b-form @submit.prevent="saveSubject">
				<b-row class="mx-auto">
					...
					<b-col cols="12" class="mt-3 text-center">
						<b-form-group >
							<b-form-radio-group
								v-model="editSubject.publicSubject"
								plain		
								size="lg">
					...
					<b-col cols="9" class="mt-2 mx-auto">
						<b-overlay :show="buttonOverlayShow" spinner-small>
							<AppButton 
								class="col-12" 
								@appButtonClick="saveSubject"
								buttonStyle="save"
								:button-disabled="!isUserOwner">
									Save Subject
							</AppButton>
						</b-overlay>
					</b-col>
					<b-col v-if="!newSubject && isUserOwner" cols="12" class="text-center mt-3 text-danger">
						<h3>
							<font-awesome-icon
...
export default {
	...
	computed: {
		...
		isUserOwner() {
			let result = false
			if(this.editSubject) result = this.editSubject.user.id == this.userId
			return result
		}
	...
	methods: {
		requestSubject(subjectId) {
			...
			.then((response) => {
				let receivedSubject = response.data
				this.editSubject = receivedSubject
				this.editSubject.userOwner = this.isUserOwner
		...
		saveSubject() {
			...
			if(!this.$v.$invalid) {
				...
				if(optionsAreValid && this.isUserOwner) {
					...
					.then(() => {
						...
						this.$notice['success']({
							title: 'Success',
							description: 'Subject has been saved',
							styles: { top: "4em" }
						})
					})
					.catch((error) => {
					...
					if(this.isUserOwner) {
						...
						this.$notice['error']({
							title: 'Attention',
							description: "You can't edit a subject which is not yours."
						})
					}
				}
			} else {
