	...
	<b-row>
		...
		<div v-show="role.role" class="mt-4">
			...
			<b-col cols="12" class="text-center mt-1">
				<router-link to="reset-password-request" style="font-weight: bold; text-decoration: none;">If you forgot your password, click here.</router-link>
			</b-col>
		</div>
	...
	mounted() {
		...
		if(this.$route.query.register_token) {
			...
			.then((response) => {
				...
				this.$notice['success']({
					title: 'Success',
					description: response.data.username + ' has been registered. You can sign in now',
					styles: { top: "4em" }
				})
			})
			.catch((error) => {
				...
				this.$notice['error']({
					title: 'Error',
					description: 'There was a problem and the user has not been registered. ' + error,
					styles: { top: "4em" }
				})
			})
		}
